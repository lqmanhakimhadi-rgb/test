...
@app.get(
    "/users/{user_id}/posts",
    response_model=List[PostResponseForUser]
)
...
async def get_all_posts():
    ...
    try:
        posts = list(
            db.posts_collection.find().sort("created_at", -1)
        )
...
async def delete_post(post_id: str):
    ...
    try:
        ...
        result = db.posts_collection.delete_one(
...
async def update_post(
    ...
    try:
        ...
        existing_post = db.posts_collection.find_one(
            {"_id": ObjectId(post_id)}
        )
